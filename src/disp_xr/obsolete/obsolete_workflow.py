...
def _get_filename_info(products_path: Path) -> pd.DataFrame:
    ...
    groupped_df = disp_df.groupby(["date1", "date2"]).apply(
...
def _write_geotiff(output_file, data, bounds, epsg=4326):
    ...
    transform = rasterio.transform.from_bounds(
        min_x, min_y, max_x, max_y, data.shape[1], data.shape[0]
    )
...
def _get_mean_quality_layers(
    ...
    client = Client(
        n_workers=n_workers, threads_per_worker=n_threads, memory_limit=memory_limit
    )
    ...
    for lyr in layers:
        ...
        print(" ", output)
        _write_geotiff(
            output, avg_data, stack_xr.rio.bounds(), stack_xr.rio.crs.to_epsg()
        )
    ...
    print(" ", output)
    _write_geotiff(
        output, zero_count, stack_xr.rio.bounds(), stack_xr.rio.crs.to_epsg()
    )
    ...
    print(" ", output)
    _write_geotiff(
        output, mask_count, stack_xr.rio.bounds(), stack_xr.rio.crs.to_epsg()
    )
    client.close()
...
def _find_reference_point(mean_quality_path: str):
    ...
    threshold = np.nanpercentile(
        np.ma.masked_array(mean_tcoh, mask=mask).filled(np.nan), 90
    )
...
def _get_chunks_indices(xr_array: xr.Dataset) -> list:
    ...
    _it, iy, ix = chunks["time"], chunks["y"], chunks["x"]
...
def _get_geometry(metadata: dict, burst_ids: list, work_dir: str, num_threads: int = 5):
    ...
    list_static_files = [
        Path(f'{static_dir}/{results[ii].properties["fileName"]}')
    ...
    _ = stitch_geometry_layers(list_static_files, output_dir=static_dir)
    ...
    _reproject_raster(
        Path(temp_dir) / "incidence_angle.tif",
        incidence_angle,
        atr,
        los_east_atr["gt"],
        metadata["GT"],
    ...
    _reproject_raster(
        Path(temp_dir) / "azimuth_angle.tif",
        az_angle,
        atr,
        los_east_atr["gt"],
        metadata["GT"],
    ...
    transformer = Transformer.from_crs(
        f'EPSG:{los_east_atr["crs"].to_epsg()}', "EPSG:4326", always_xy=True
    )

    # Find bounds
    snwe = np.zeros((4))
    snwe[2], snwe[1] = transformer.transform(
        atr["bounds"].left - 1e3, atr["bounds"].top + 1e3
    )
    snwe[3], snwe[0] = transformer.transform(
        atr["bounds"].right + 1e3, atr["bounds"].bottom - 1e3
    )
    ...
    # DEM
    print("Download DEM")
    dat_arr, dat_prof = dem_stitcher.stitch_dem(
        bounds,
        dem_name="glo_30",
    ...
    X, p = tile_mate.get_raster_from_tiles(
...
def create_timeseries_h5(
    ...
    meta = get_metadata(
        disp_df.path.iloc[0], disp_df.start_date.min().strftime("%Y%m%d")
    )
    ...
    for ix, date in enumerate(reference_dates):
        ...
        stacks.append(
            xr.open_mfdataset(stack_files, chunks={"time": -1, "x": 512, "y": 512})
        )
        if ix > 0:
    ...
    _get_mean_quality_layers(
        stack,
        output_dir / "mean_quality_layers",
        n_workers=10,
        n_threads=10,
        memory_limit="4GB",
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Process and write OPERA DISP to MintPy container."
    )
    ...
    parser.add_argument(
        ...
        type=list,
        default=None,
        help="Spatial reference [latitute, longitude].",
    )
    ...
    create_timeseries_h5(
        args.disp_nc, args.output_dir, args.ref_lalo, args.mask, True, args.num_threads
    )
